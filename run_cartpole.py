...
parser.add_argument('--train_steps', default=1e3, type=float, help='number of training steps per each training epoch')
...
parser.add_argument('--final_eps', default=1e-3, type=float, help='final exploration rate')
...
def pre_process(obs):
    # Discretize continuous state representation into buckets in one-hot manner
    buckets = (10,10,10,10)
    # Upper and lower bound of [Postition, Velocity, Angle, Angle velocity]
    upper_bounds = [ 2.5,  0.5,  0.28,  0.88]
    lower_bounds = [-2.5, -0.5, -0.28, -0.88]
    ratios = [(obs[i] + abs(lower_bounds[i])) / (upper_bounds[i] - lower_bounds[i]) for i in range(4)]
    new_obs = [int(round((buckets[i] - 1) * ratios[i])) for i in range(len(ratios))]
    new_obs = [min(buckets[i] - 1, max(0, new_obs[i])) for i in range(len(ratios))]
    idx = torch.tensor(new_obs)
    return torch.zeros(4, 10).scatter_(1, idx.unsqueeze(1), 1.).view(-1)
...
def learn(env):
    ...
    model_spike = model_lsm.LSM(f=f,
            ...
            inp_size=40, rate_scale=args.rate_scale, 
